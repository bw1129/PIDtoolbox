...
TooltipString_crossAxesStats=['Selects from several plotting options, from basic histograms of stick use per flight, to means',...
    newline 'and various between-axes representations (''Mode and axes X throttle topography plots'').',... 
    newline 'Line color in Mode 1-2 plots represent throttle acceleration in red and deceleration in blue,',...
    newline 'with higher values more saturated. If throttle is neither accelerating nor decelerating the line color is white.',...
    newline 'The ''Mode and axes X throttle topography plots'' are useful for examining patterns in flight behavior,',...
    newline 'For example, flight behaviour associated with optimal lap times in a race, ',...
    newline ' or the qualities or asymmetries in one''s flying style'];
...
guiHandlesStats.crossAxesStats =uicontrol(PTstatsfig,'Style','popupmenu','String',{'Histograms'; 'Mean +/-Standard Error of the Mean'; 'Mode 1 topography'; 'Mode 2 topography'; 'Axes X Throttle'},'fontsize',fontsz5,'TooltipString',[TooltipString_crossAxesStats],...
